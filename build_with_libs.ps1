# Album Cleaner Build Script with OkHttp
...
Write-Host "Album Cleaner APK Builder (with OkHttp)" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
# Get all library JARs
$LIB_JARS = Get-ChildItem -Path "$APP_DIR\libs" -Filter "*.jar" | Select-Object -ExpandProperty FullName
$CLASSPATH = "$PLATFORM\android.jar;$APP_DIR\build\intermediates"
if ($LIB_JARS) {
    $CLASSPATH += ";" + ($LIB_JARS -join ";")
}
...
& "$JAVA_BIN\javac.exe" -encoding UTF-8 -cp $CLASSPATH -d "$APP_DIR\build\classes" $javaFiles
if ($LASTEXITCODE -ne 0) { Write-Error "Java compilation failed"; exit 1 }

# Step 4: Convert to DEX (include library classes)
...
$dexInputs = $classFiles
if ($LIB_JARS) {
    $dexInputs = $classFiles + $LIB_JARS
}
& "$BUILD_TOOLS\d8.bat" --release --output "$APP_DIR\build\intermediates" $dexInputs
...
Copy-Item "$APP_DIR\build\outputs\app-release.apk" "$APP_DIR\build\outputs\apk\debug\app-debug.apk" -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path "$APP_DIR\build\outputs\apk\debug" -Force | Out-Null
Copy-Item "$APP_DIR\build\outputs\app-release.apk" "$APP_DIR\build\outputs\apk\debug\app-debug.apk" -Force

Write-Host "`n=========================================" -ForegroundColor Green
Write-Host "Build Successful!" -ForegroundColor Green
Write-Host "APK: $APP_DIR\build\outputs\app-release.apk" -ForegroundColor Green
Write-Host "Debug APK: $APP_DIR\build\outputs\apk\debug\app-debug.apk" -ForegroundColor Green
